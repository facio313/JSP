...
//	@Test
	public void classLoaderTest() throws ClassNotFoundException, IOException {
		ClassLoader loader = ClassLoader.getSystemClassLoader(); // 이 로더가 클래스르 메모리에 적재하는 역할을 함
		Class<?> clz = loader.loadClass("kr.or.ddit.servlet01.DescriptionServlet"); // 상수 메모리에 적재됨
...
//		FileInputStream fis = new FileInputStream(file); // byte
		try(FileReader fr = new FileReader(file);){// char
			int tmp = -1;
			while((tmp=fr.read()) != -1) {
...
//	4. 개방된 스틀미 종료(매체를 해제시켜 다른 쓰레드가 사용할 수 있도록 함)
...
//		ResourceBundle bundle = ResourceBundle.getBundle(baseName); // locale-specific이라 한글로 받아옴
