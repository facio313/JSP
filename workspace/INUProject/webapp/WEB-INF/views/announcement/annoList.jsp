...
// JSON.stringify(data)
let tagBody = $("#tagBody");
// let regionTag = function(event){
//    return $("<div>").attr("class",event.regionCode).html(event.regionName);
// }

let regionBtn = $("[name=regionBtn]").on("click", function(event){
   let type = 'region';
   let code = '';
   let data = {type:type, code:code};
   $.ajax({
      ...
      success : function(resp) {
         console.log("돌아왓다");
         let list = resp.searchList;
         tagBody.empty();
         let trTags = [];
         //클래스 붙여주기
         $.each(list, function(index, val){
            let tr = null;
            console.log(val.regionCode);
            console.log(val.regionName);
            tr = $("<li>").append($("<button>").attr("class","code").prop("value",val.regionCode).html(val.regionName));
            trTags.push(tr);
         })
         tagBody.html(trTags);
      },
