...
public class PdfServiceImpl implements PdfService {
	...
	public String createPdf(String keyword) {
		...
		try {
			...
			PdfPCell cell1 = new PdfPCell(new Phrase("책 번호", font)); // 셀의 이름과 폰트를 지정해서 셀을 생성한다.
			...
			PdfPCell cell3 = new PdfPCell(new Phrase("카테고리", font));
			cell3.setHorizontalAlignment(Element.ALIGN_CENTER);

			PdfPCell cell4 = new PdfPCell(new Phrase("금액", font));
			cell4.setHorizontalAlignment(Element.ALIGN_CENTER);

			PdfPCell cell5 = new PdfPCell(new Phrase("등록날짜", font));
			...
			AnnoVO anno = annoService.retrieveAnno(keyword);
				PdfPCell cellProductName = new PdfPCell(new Phrase("" + anno.getAnnoNo(), font)); // 반복문을 사용해서 상품정보를 하나씩
																									// 출력해서 셀에 넣고 테이블에
																									// 저장한다.
				...
				/*
				PdfPCell cellAmount = new PdfPCell(new Phrase(bvo.getCategory(), font));
				// Phrase타입은 숫자형(int형 같은타입)으로 하면 에러가 발생되기 때문에 dto앞에 공백("")주어서 String타입으로 변경한다.

				PdfPCell cellMoney = new PdfPCell(new Phrase("" + bvo.getPrice(), font));
				// Phrase타입은 숫자형(int형 같은타입)으로 하면 에러가 발생되기 때문에 dto앞에 공백("")주어서 String타입으로 변경한다.

				PdfPCell cellDate = new PdfPCell(new Phrase("" + bvo.getInsertDate(), font));
				// Phrase타입은 숫자형(int형 같은타입)으로 하면 에러가 발생되기 때문에 dto앞에 공백("")주어서 String타입으로 변경한다.
				 */
